...
$('.search').on("click", function (event) {
	...
	location = $(this).parent('.btnPar').siblings('.textVal').val().trim();
	if (location === "") {
		return;
	};
	locationHist.push(location);

	localStorage.setItem('location', JSON.stringify(locationHist));
...
var contHistEl = $('.locationHist');
function getHistory() {
	contHistEl.empty();

	for (let i = 0; i < locationHist.length; i++) {

		var rowEl = $('<row>');
		var btnEl = $('<button>').text(`${locationHist[i]}`)
...
function getWeatherToday() {
	var getUrlCurrent = `https://api.openweathermap.org/data/2.5/weather?q=${location}&units=imperial&appid=${key}`;
	...
	$.ajax({
		...
		//Set the lat and long from the searched location
...
function getFiveDayForecast() {
	var getUrlFiveDay = `https://api.openweathermap.org/data/2.5/forecast?q=${location}&units=imperial&appid=${key}`;
